class Star{
  constructor(){
    ...
    render(){
        ...
        drawStar(this.position[0],this.position[1],this.numSide,this.size,this.array,this.arrayPoint);
    }
  }

function drawStar(x,y,n,size,array,pointArray)
{
  //let new_array = the270DegreeRotationArray(array,[x,y]);
  let new_array = array;
  for(let i = 0; i < pointArray.length;i++)
  {
    drawTriangle(pointArray[i])
  }
...
function generatePointStarVertices(x,y,n,array)
{
  let returnArray = [];
  let new_array = array;
  for(let i = 0; i < n-1; i++)
  {
    ...
    returnArray.push([new_array[i][0],new_array[i][1],  last_point[0],last_point[1],  new_array[i+1][0],new_array[i+1][1]]);
  }
  ...
  returnArray.push([new_array[0][0],new_array[0][1],  last_point[0],last_point[1],  new_array[new_array.length-1][0],new_array[new_array.length-1][1]]);
